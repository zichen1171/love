    ...
    const canvas = document.getElementById('cosmic-stage');
    ...
    function registerTick(callback) {
        if (typeof callback === 'function') {
            tickSubscribers.push(callback);
            return () => {
                const index = tickSubscribers.indexOf(callback);
                if (index !== -1) {
                    tickSubscribers.splice(index, 1);
                }
            };
        }
        return () => {};
...
window.cosmicStage = cosmicStageApi;
window.dispatchEvent(new CustomEvent('cosmic-stage-ready', {detail: cosmicStageApi}));
    ...
    const currentAngle =
        ...
        tickSubscribers.forEach((fn) => {
            try {
                ...
                console.error('cosmic-stage subscriber error', error);
            }
        });
